...
function GalleryPageContent({ experience, projects }: { experience: ExperienceConfig; projects: Project[] }) {
  ...
  const categories = useMemo(() => {
    return ['All', 'Construction Photos', 'My Photos', 'Plans & Designs'];
  }, []);
          ...
          <ScrollReveal delay={0.1}>
            ...
            <p className="font-sans text-lg text-white/70 max-w-2xl mx-auto">
              {experience.type === 'interiors'
                ? 'A curated visual journey through our residential and commercial interiors.'
          ...
          <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
            {filteredImages.map((image, index) => (
              <motion.div
                ...
                title="" // <--- ADDED THIS: Suppresses tooltip on the container
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
              ...
              >
                <div className="relative w-full" style={{ paddingBottom: index % 3 === 0 ? '125%' : index % 2 === 0 ? '75%' : '100%' }}>
                  <Image
                    src={image.src}
                    alt={image.alt}
                    title="" // <--- ADDED THIS: Suppresses native browser hover tooltip on the image
                    fill
...
export default function ConstructionGalleryPage() {
  return <GalleryPageContent experience={constructionConfig} projects={constructionProjects} />;
}