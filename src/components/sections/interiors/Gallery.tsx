...
export function Gallery({ experience, projects, maxItems = 6, number = '02' }: GalleryProps) {
  // Grab up to 6 images for the grid (removed the 'featured' filter so it guarantees 6 items)
  ...
  return (
    <section id="gallery" className="section bg-white py-24 lg:py-36 border-b border-neutral-100 text-black" aria-labelledby="gallery-heading">
      <div className="container-lg mx-auto px-4 md:px-6">
        ...
        {/* CUSTOM HEADER SECTION (Forced Black/Dark Text for Visibility) */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12 lg:mb-16">
          <ScrollReveal delay={0.1} className="max-w-2xl">
            {/* Number & Eyebrow (e.g., 02 ---- Home Interiors) */}
...
function GalleryCard({ item, experience }: GalleryCardProps) {
  return (
    <motion.a
      ...
      whileHover="hover"
      initial="initial"
      title="" // Prevents the browser's default tooltip from showing on hover
    >
      {/* Background Image */}
      <Image
        src={item.coverImage}
